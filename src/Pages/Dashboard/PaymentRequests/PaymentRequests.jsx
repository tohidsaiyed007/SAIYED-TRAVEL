...
function PaymentRequests() {
  ...
  const handleExportExcel = () => {
    if (
      ...
      alert(
        "No payment requests available to export."
      );
      return;
    }
  ...
  const handleDeleteRequest =
    async (request) => {
      ...
      try {
        ...
        const response =
          await fetch(
            `${API_URL}/api/payment-requests/${request._id}`,
            {
              method: "DELETE",
              headers: {
                Authorization:
  ...
  const handleDeleteSelected =
    async () => {
      if (
        ...
        alert(
          "Please select at least one payment request."
        );
        return;
      }
      ...
      if (
        !window.confirm(
          `Are you sure you want to delete ${selectedIds.length} selected payment request(s)?`
        )
      ) {
        return;
      }
      ...
      try {
          ...
          await Promise.allSettled(
            selectedIds.map(
              async (id) => {
                const response =
                  await fetch(
                    `${API_URL}/api/payment-requests/${id}`,
                    {
                      method:
                        "DELETE",
                      headers: {
        ...
        alert(
          failed === 0
            ? `${successful} payment request(s) deleted successfully.`
            : `${successful} deleted successfully.\n${failed} could not be deleted.`
        );
      } catch (error) {
        console.error(
          "DELETE SELECTED PAYMENT REQUESTS ERROR:",
          error
        );

        alert(
          error.message ||
            "Unable to delete selected payment requests."
        );
      } finally {
        setDeleting(false);
      }
    ...
    <div className="payment-requests-page">
      ...
      <div className="payment-page-header">
        ...
        <div className="payment-header-actions">
          ...
          <button
            type="button"
            className="delete-selected-payment-btn"
            onClick={
