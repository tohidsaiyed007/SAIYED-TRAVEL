...
function MyBookings() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      // Remove selected IDs which no longer exist
      setSelectedIds((prev) =>
        prev.filter((id) =>
          bookingData.some(
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      if (failed.length > 0) {
        alert(
          `${successful} booking(s) deleted successfully. ${failed.length} booking(s) failed.`
        );
      } else {
        alert(
          `${successful} booking(s) deleted successfully.`
        );
      }
      ...
      // Close modal if deleted booking was open
      setSelectedBooking(null);
      ...
      console.error(
        "Delete Selected Bookings Error:",
        error
      );

      alert(
        error.message ||
          "Unable to delete selected bookings."
      );
    } finally {
      setDeleting(false);
    }
